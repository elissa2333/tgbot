...
// OptionalWebhook 可选参数
type OptionalWebhook struct {
	Certificate    io.Reader `json:"certificate,omitempty"`     // 上传您的公共密钥证书，以便可以检查正在使用的根证书。有关详细信息，请参见我们的自签名指南。
...
func (a *API) SetWebhook(url string, optional *OptionalWebhook) error { // TODO 仅允许 telegram 的 ip 进行访问 我直接监听指定的URI
...
func (a API) DeleteWebhook() error {
	res, err := a.HTTPClient.Post("/deleteWebhook")
	if err != nil {
		return err
	}
	defer res.Body.Close()
